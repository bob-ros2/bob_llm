...
def load_env(file_path):
    ...
    if not os.path.exists(file_path):
        template_path = os.path.join(os.path.dirname(file_path), '.env.template')
        msg = (
            "\n[bold red]❌ ERROR: Configuration file '.env' not found![/bold red]\n"
            f"To run this integration test, you must create a [bold].env[/bold] file in:\n"
            f"[blue]{os.path.dirname(file_path)}[/blue]\n\n"
            "1. Copy the template:\n"
            f"   [cyan]cp {template_path} {file_path}[/cyan]\n\n"
            "2. Edit the [bold].env[/bold] file and set your [yellow]LLM_API_KEY[/yellow].\n"
            "3. Ensure [yellow]LLM_TOOL_INTERFACES[/yellow] points to a valid absolute path.\n"
        )
        if HAS_RICH:
            console.print(msg)
        else:
        ...
        pytest.exit("Missing .env configuration file. See instructions above.", returncode=1)
...
class LLMTestNode(Node):
    ...
    def tool_calls_cb(self, msg):
        if self.verbose:
            if HAS_RICH:
                console.print(f"[bold cyan]🔧 TOOL CALL:[/bold cyan] [yellow]{msg.data}[/yellow]")
            else:
...
def test_full_mandatory_flow(ros_context, llm_node_process, test_config):
    """
    Comprehensive test of all mandatory LLM topics with deferred beautiful output.
    """
    ...
    # We add a unique timestamp/instruction to prevent caching/hallucination of "I already know this"
    prompt = "Check the live ROS topic list right now and tell me exactly how many topics are active. Do NOT rely on your memory, I need fresh data via your tools."
    
    print("\n")
    if HAS_RICH:
        console.print(f"[bold magenta]───────────────── STARTING INTEGRATION TEST (Domain 88) ─────────────────[/bold magenta]")
        console.print(f"[bold]Prompt:[/bold] {prompt}")
        console.print(f"[bold]Mode:[/bold]   {'REASONING' if is_reasoner else 'STANDARD'}")
        console.print(f"[bold magenta]─────────────────────────────────────────────────────────────────────────[/bold magenta]")
    else:
        print(f"\n" + "="*50)
        print(f"--- STARTING INTEGRATION TEST (Domain 77) ---")
    ...
    while time.time() - start_time < timeout:
        ...
        if response_received_time and (time.time() - response_received_time > 3.0):
            break
    ...
    if verbose:
        if HAS_RICH:
            ...
            if test_node.stream_tokens:
                ...
                console.print(f"\n[bold green]📝 Streamed Output (Markdown)[/bold green]")
                console.print(Markdown(stream_content))
            
            if test_node.responses:
                console.print(f"\n[bold white]✅ Final Response Topic[/bold white]")
                console.print(Markdown(test_node.responses[-1]))
        else:
            print("\n" + "-"*50)
            print("--- FULL CONTENT LOG (DEFERRED OUTPUT) ---")
            if test_node.reasoning_chunks:
                print("\n[REASONING CONTENT]:\n" + "".join(test_node.reasoning_chunks))
            if test_node.stream_tokens:
                print("\n[STREAMED TOKENS]:\n" + "".join(test_node.stream_tokens))
            if test_node.responses:
    ...
    print(f"[CHECK] llm_stream (Tokens):   {'PASSED' if stream_ok else 'FAILED'} ({len(test_node.stream_tokens)} tokens)")
    
    response_ok = len(test_node.responses) > 0
    print(f"[CHECK] llm_response (Final):  {'PASSED' if response_ok else 'FAILED'}")
    
    reasoning_ok = len(test_node.reasoning_chunks) > 0
    if is_reasoner:
        print(f"[CHECK] llm_reasoning (MANDATORY): {'PASSED' if reasoning_ok else 'FAILED'}")
    else:
        print(f"[INFO]  llm_reasoning (Optional):  {'RECEIVED' if reasoning_ok else 'NOT RECEIVED'}")
        
    tools_ok = len(test_node.tool_calls) > 0
    print(f"[CHECK] llm_tool_calls (Tools): {'PASSED' if tools_ok else 'FAILED'}")
    ...
    if is_reasoner:
        assert reasoning_ok, "MANDATORY: Model is reasoner but no reasoning received"
